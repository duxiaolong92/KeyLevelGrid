...
class GateKlineFeed:
    ...
    async def _fetch_klines(
        ...
        Gate API: GET /api/v4/futures/usdt/candlesticks
        
        参数:
        - contract: 合约名称 (如 BTC_USDT)
        - interval: K线周期 (1m, 5m, 15m, 30m, 1h, 4h, 1d, 7d)
        - limit: 数量限制 (默认 100，最大 2000)
        """
        # 限频
        ...
        params = {
            ...
            "interval": interval,
            "limit": min(limit, 2000),  # Gate 最大 2000
        }
        ...
        try:
            async with self.session.get(url, params=params) as resp:
                ...
                # Gate 返回格式: [{t, v, c, h, l, o, sum}, ...]
                # t: 时间戳（秒）, v: 成交量, c: 收盘, h: 最高, l: 最低, o: 开盘, sum: 成交额
                # Gate 返回数据是时间升序（最旧在前），无需反转
                klines = []
                for item in data:
                    try:
                        kline = Kline(
                            timestamp=int(item["t"]) * 1000,  # 转换为毫秒
                            open=float(item["o"]),
                            high=float(item["h"]),
                            low=float(item["l"]),
                            close=float(item["c"]),
    ...
    async def _handle_ws_message(
        ...
        for result in result_list:
            try:
                # 解析 K 线
                # 文档字段: t=时间戳(秒), o=开盘, h=最高, l=最低, c=收盘, v=成交量, a=成交额, w=是否收盘
                timestamp = int(result["t"]) * 1000  # 转为毫秒
                is_closed = result.get("w", False)  # w=true 表示 K 线已收盘
                ...
                kline = Kline(
                    ...
                    quote_volume=float(result.get("a", 0)),  # a=成交原始币种数量
                    trades=0,
                    is_closed=is_closed,
                )
